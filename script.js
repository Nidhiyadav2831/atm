...
// Bump this to invalidate previously saved localStorage data when initial users change
...
// Sample user data
const initialUsers = [
    { accountNumber: "1234567890", pin: "1234", name: "John", balance: 5000.00 },
    { accountNumber: "0987654321", pin: "5678", name: "Ram", balance: 7500.00 },
    { accountNumber: "1122334455", pin: "1111", name: "shyam", balance: 12500.00 },
    { accountNumber: "2233445566", pin: "2222", name: "Aman", balance: 3000.00 },
    { accountNumber: "3344556677", pin: "3333", name: "Naina", balance: 8900.00 },
    { accountNumber: "4455667788", pin: "4444", name: "Akhi", balance: 15000.00 },
    { accountNumber: "5566778899", pin: "5555", name: "Yatin", balance: 6800.00 },
    { accountNumber: "6677889900", pin: "6666", name: "Geeta", balance: 4200.00 },
    { accountNumber: "7788990011", pin: "7777", name: "Suresh", balance: 11000.00 },
    { accountNumber: "8899001122", pin: "8888", name: "Seeta", balance: 9500.00 }
];
...
function initializeATM() {
    ...
    if (userHashTable.getAllEntries().length === 0) {
        ...
        console.log("Users:", userHashTable.getAllEntries().map(([acc, user]) => `${user.name} (PIN: ${user.pin})`).join(', '));
...
function processWithdraw() {
    ...
    successDiv.textContent = `Withdrawal successful! ${formatCurrency(amount)} withdrawn.`;
...
function processDeposit() {
    ...
    const successDiv = document.createElement('div');
    successDiv.className = 'success-message';
    successDiv.textContent = `Deposit successful! ${formatCurrency(amount)} deposited.`;
    successDiv.style.cssText = 'background: #4CAF50; color: white; padding: 10px; margin: 10px 0; border-radius: 5px; text-align: center;';
...
function processTransfer() {
    ...
    const successDiv = document.createElement('div');
    successDiv.className = 'success-message';
    successDiv.textContent = `Transfer successful! ${formatCurrency(amount)} sent to ${recipientData.name}.`;
    successDiv.style.cssText = 'background: #4CAF50; color: white; padding: 10px; margin: 10px 0; border-radius: 5px; text-align: center;';
    ...
    headerDiv.innerHTML = `
        <div class = "header-info">
            ...
            <div class="current-balance">
                <strong>Current Balance: ${formatCurrency(currentUser.balance)}</strong>
            </div>
        </div>
    ...
    sortedTransactions.forEach((transaction, index) => {
        ...
        transactionDiv.innerHTML = `
            <div class="transaction-info">
                ...
                <div class="transaction-description" style="color: ${transactionColor};">${description} - ${formatCurrency(Math.abs(transaction.amount))}</div>
            ...
            <div class="transaction-details">
                ...
                <div class="transaction-balance">Balance: ${formatCurrency(transaction.balance)}</div>
            </div>
        `;
...
function loadUserData() {
    ...
    if (savedData) {
        ...
        // If saved data version doesn't match, skip loading to allow fresh initialization
        if (!data.version || String(data.version) !== String(DATA_VERSION)) {
            console.log('Saved data version mismatch. Expected', DATA_VERSION, 'but found', data.version, '. Skipping load and reinitializing.');
            return;
        }
